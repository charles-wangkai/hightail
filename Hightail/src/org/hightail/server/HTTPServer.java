...
/**
 * This class makes it possible for Hightail to pick up tasks sent via HTTP requests. This makes it
 * possible for browser extensions to parse tasks in the browser and send them to a running Hightail
 * instance.
 *
...
public class HTTPServer {
  /**
   * CHelper runs on port 4243. By running Hightail on a different port it's possible to use both
  ...
  /**
   * Start the HTTP server and start listening for tasks. If the port is already occupied, it
   * doesn't do anything.
   */
   ...
   *
   * <p>Does three things: 1. Check if the request is a POST request. 2. Attempt to parse the body
   * of the request. 3. If successful, add the task with it's tests.
   *
   * @param exchange
   */
